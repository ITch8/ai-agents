...
import { TradeDecisionRequest } from '@/services/types';
...
export function useFlowConnection(flowId: string | null) {
  ...
  const runFlow = useCallback((params: TradeDecisionRequest) => {
    ...
    try {
      // Start the API call
      const abortController = api.runTradeDecision(params, nodeContext, flowId);
      ...
      console.error('Failed to start decision run:', error);
      flowConnectionManager.setConnection(flowId, {
        state: 'error',

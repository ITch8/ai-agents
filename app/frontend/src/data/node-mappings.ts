...
const baseNodeTypeDefinitions: Record<string, NodeTypeDefinition> = {
  "Trade Input": {
    createNode: (position: { x: number, y: number }): AppNode => ({
      ...
      data: {
        name: "Trade Input",
        description: "Enter your foreign-trade question context and connect to expert agents.",
        status: "Idle",
      },
    }),
  },
  "Decision Node": {
    createNode: (position: { x: number, y: number }): AppNode => ({
      id: `trade_decision_${generateUniqueIdSuffix()}`,
      type: "portfolio-manager-node",
      position,
      data: {
        name: "Decision Node",
        description: "Generates final foreign-trade recommendation based on all agent outputs.",
        status: "Idle",
      },
    }),
  },
  "Product Input": {
    createNode: (position: { x: number, y: number }): AppNode => ({
      ...
      data: {
        name: "Product Input",
        description: "Enter candidate product/category and connect this node to agents for analysis.",
        status: "Idle",
      },
    }),
  },

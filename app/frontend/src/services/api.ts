...
export const api = {
  ...
  getAgents: async (): Promise<Agent[]> => {
    try {
      const response = await fetch(`${API_BASE_URL}/trade-decision/agents`);
      if (!response.ok) {
  ...
  /**
   * Runs trade decision flow and streams progress/results.
   */
  runTradeDecision: (
    params: TradeDecisionRequest,
    nodeContext: ReturnType<typeof useNodeContext>,
    flowId: string | null = null
  ): (() => void) => {
    // Convert tickers string to array if needed (legacy compatibility)
    if (typeof params.tickers === 'string') {
      params.tickers = (params.tickers as unknown as string).split(',').map(t => t.trim());
    }

    // Ensure required legacy fields exist while backend has mixed schema support.
    if (!params.tickers || params.tickers.length === 0) {
      params.tickers = ['TRADE'];
    }
    ...
    fetch(`${API_BASE_URL}/trade-decision/run`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  runHedgeFund: (
    ...
    nodeContext: ReturnType<typeof useNodeContext>,
    flowId: string | null = null
  ) => api.runTradeDecision(params, nodeContext, flowId),
}; 